...
# Inisialisasi Session State (Agar data tetap sinkron)
...
<style>
    ...
    {f'.login-bg {{ background-image: linear-gradient(rgba(2, 132, 199, 0.7), rgba(2, 132, 199, 0.7)), url("data:image/png;base64,{BG_BASE64}"); background-size: cover; position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; }}' if BG_BASE64 else ''}
...
if st.session_state['role'] is None:
    ...
    with st.container():
        ...
        if col_l1.button("👤 WALI MURID / USER", use_container_width=True): st.session_state['role'] = 'user'; st.rerun()
        if col_l2.button("🔑 ADMINISTRATOR", use_container_width=True): st.session_state['role'] = 'admin_auth'; st.rerun()
...
if st.session_state['role'] == 'admin_auth':
    ...
    if st.button("⬅️ Kembali"): st.session_state['role'] = None; st.rerun()
...
with st.sidebar:
    ...
    nav_list = ["🏠 Profil Sekolah", "📝 Pendaftaran Siswa Baru", "📋 Daftar Siswa Terdaftar", "📸 Galeri Sekolah", "👨‍🏫 Profil Guru & Staf"]
    if st.session_state['role'] == 'admin': nav_list.append("🔐 Panel Admin")
    menu = st.selectbox("NAVIGASI UTAMA", nav_list)
    if st.button("🚪 Log Out"): st.session_state['role'] = None; st.session_state['auth'] = False; st.rerun()
...
if menu == "🏠 Profil Sekolah":
    ...
    if st.session_state['role'] == 'admin' and c_edit_btn.button("✏️ Edit"): st.session_state['is_editing_prof'] = True
    if st.session_state.get('is_editing_prof', False) and st.session_state['role'] == 'admin':
        ...
        with col_p1: st.markdown(f"""<table class="emis-table"><tr><td class="label-emis">KEPALA MADRASAH</td><td>: {st.session_state['INFO_LEMBAGA']['Kepala']}</td></tr><tr><td class="label-emis">ALAMAT</td><td>: {st.session_state['INFO_LEMBAGA']['Alamat']}</td></tr></table>""", unsafe_allow_html=True)
        with col_p2: st.markdown(f"""<table class="emis-table"><tr><td class="label-emis">PROVINSI</td><td>: {st.session_state['INFO_LEMBAGA']['Provinsi']}</td></tr><tr><td class="label-emis">KABUPATEN</td><td>: {st.session_state['INFO_LEMBAGA']['Kabupaten']}</td></tr></table>""", unsafe_allow_html=True)
...
# 2. PENDAFTARAN (LENGKAP 37 KOLOM & TAHUN 1945-2100)
    ...
    with st.form("ppdb_full_form", clear_on_submit=True):
        ...
        prov, kota = c1.text_input("Provinsi", value="Jawa Timur"), c2.text_input("Kota", value="Kediri")
        ...
        if st.form_submit_button("✅ KIRIM"):
            if nama and nik_s and no_wa:
                try:
                    ...
                    row = [reg_id, nama, nisn, nis_lokal, kwn, f"'{nik_s}", str(tgl_s), tmp_s, jk, saudara, anak_ke, agama, f"'{no_kk}", nama_kepala_kk, wa_fix, n_ay, f"'{nik_ay}", "", str(tgl_ay), "", pek_ay, gaj_ay, n_ib, f"'{nik_ib}", "", str(tgl_ib), "", pek_ib, gaj_ib, st_rmh, prov, kota, "", "", alamat, pos, datetime.now().strftime("%Y-%m-%d"), "Belum Diverifikasi"]
...
# 3. DAFTAR SISWA (HITUNG UMUR & AKSES TERPISAH)
    ...
    try:
        ...
        if data:
            ...
            display_df['NAMA'] = df['Nama Lengkap']
            display_df['TEMPAT LAHIR'] = df['Tempat Lahir']
            display_df['TANGGAL LAHIR'] = df['Tanggal Lahir']
            display_df['UMUR'] = display_df['TANGGAL LAHIR'].apply(lambda x: hitung_umur(str(x)))
            st.table(display_df)

            if st.session_state['role'] == 'admin':
                st.divider()
                st.subheader("✏️ Koreksi Cepat (Admin)")
                sel_siswa = st.selectbox("Pilih Siswa", df['Nama Lengkap'].values)
                idx_row = df.index[df['Nama Lengkap'] == sel_siswa].tolist()[0] + 2
                with st.form("q_edit"):
                    new_n = st.text_input("Ubah Nama", value=sel_siswa)
...
# 4. GALERI
    ...
    if st.session_state['role'] == 'admin':
        with st.expander("📤 Upload Foto"):
            files = st.file_uploader("Pilih Gambar", accept_multiple_files=True)
            txt = st.text_input("Deskripsi")
            if st.button("Simpan"):
                for f in files:
                    st.session_state['temp_gallery'].append({"img": base64.b64encode(f.getvalue()).decode(), "desc": txt})
                st.rerun()
    cols = st.columns(3)
...
# 5. GURU & STAF
    ...
    if st.session_state['role'] == 'admin':
        with st.expander("➕ Tambah Guru"):
            with st.form("g"):
                n, j = st.text_input("Nama"), st.text_input("Jabatan")
                p, b = st.file_uploader("Foto"), st.text_area("Bio")
    ...
    for idx, s in enumerate(st.session_state['staff_data']):
        with cols_s[idx % 3]:
            st.markdown(f'<div class="staff-card"><img src="data:image/png;base64,{s["photo"]}" style="width:100%; border-radius:10px;"><div style="color:#0284C7; font-weight:bold;">{s["name"]}</div><div>{s["job"]}</div><div style="font-size:12px;">{s["bio"]}</div></div>', unsafe_allow_html=True)
            if st.session_state['role'] == 'admin' and st.button("🗑️", key=idx): st.session_state['staff_data'].pop(idx); st.rerun()
